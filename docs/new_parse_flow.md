...
### 🔥 Итерация 1: Single File Tail Reader
**Срок:** Неделя 1  
**Цель:** Реализовать streaming обработку с tail-like подходом
...
└── single_file_tail_watcher.py  # НОВЫЙ - streaming tail reader
```

#### Реализация SingleFileTailWatcher:
```python
...
#### Тестирование:
- [x] Сравнение времени обработки: legacy vs single file tail
- [x] Проверка дублирования ордеров
- [x] Мониторинг производительности
- [x] Покрытие тестами: 16 тестов для SingleFileTailWatcher
